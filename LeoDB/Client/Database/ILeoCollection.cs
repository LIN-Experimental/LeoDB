...
public interface ILeoCollection<T>
{
    /// <summary>
    /// Get collection name
    /// </summary>
    ...
    /// </summary>
    ILeoCollection<T> Include<K>(Expression<Func<T, K>> keySelector);

    /// <summary>
    /// Run an include action in each document returned by Find(), FindById(), FindOne() and All() methods to load DbRef documents
    /// Returns a new Collection with this action included
    /// </summary>
    ILeoCollection<T> Include(BsonExpression keySelector);
    ...
    /// </summary>
    IEnumerable<T> FindVectorial(Query query, string find, int skip = 0, int limit = int.MaxValue);
